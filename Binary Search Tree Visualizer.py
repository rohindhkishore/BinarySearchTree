...
class BST:
    def __init__(self, root):
        ...
        self.make_canvas = Canvas(self.window,width=2500,height=600,bg="pink",relief=RAISED,bd=8)
        self.make_canvas.place(x=0,y=0)
        # Canvas make for result presentation
        self.result_canvas = Canvas(self.window,width=1340,height=168,bg="sky blue",relief=RAISED,bd=4)
    ...
    def heading_root_and_null_make(self):# By default window presentation
        make_heading = Label(self.make_canvas,text="Binary Search Tree Visualizer",font=("Times New Roman",27,"bold","italic"),fg="brown",bg="pink")
        make_heading.place(x=380,y=10)
        make_root = Label(self.make_canvas,text="Root",font=("Times New Roman",17,"bold"),fg="black",bg="pink")
        ...
        self.make_canvas.create_polygon(root_indicator, width=3, fill="white", outline="purple")

        self.make_null = Label(self.make_canvas,text="NULL",font=("Times New Roman",17,"bold","italic"),fg="#2E37FE",bg="pink")
        self.make_null.place(x=565,y=90+50)

    def make_instructional_buttons(self):# For make Instructional buttons
        #Insert button
        self.insert_node = Button(self.window,text="Insert",font=("Times New Roman",20,"bold"),bg="salmon",fg="black",relief=RAISED,bd=8,command=lambda: self.take_input_set_up(1),state=NORMAL)
        self.insert_node.place(x=30,y=50)

        #LabelFrame make for Traversing Buttons
        make_label_frame_for_traversing = LabelFrame(self.window,text="Traversing",width=100,bg="pink",font=("Times New Roman",13,"bold"),fg="black")
        make_label_frame_for_traversing.place(x=1150,y=200)

        self.pre_order_traversal  = Button(make_label_frame_for_traversing,text="Pre-order ",font=("Times New Roman",13,"bold"),bg="salmon",fg="black",relief=RAISED,bd=8,command=lambda: self.traversing_decision_making(1),state=NORMAL)
        self.pre_order_traversal.pack(padx=10,pady=5)

        self.in_order_traversal = Button(make_label_frame_for_traversing,text=" In-order ",font=("Times New Roman", 13,"bold"),bg="salmon",fg="black",relief=RAISED,bd=8,command=lambda: self.traversing_decision_making(2),state=NORMAL)
        self.in_order_traversal.pack(padx=10, pady=5)

        self.post_order_traversal = Button(make_label_frame_for_traversing,text="Post-order",font=("Times New Roman", 13,"bold"),bg="salmon",fg="black",relief=RAISED,bd=8,command=lambda: self.traversing_decision_making(3),state=NORMAL)
        self.post_order_traversal.pack(padx=10, pady=5)

        self.level_order_traversal = Button(make_label_frame_for_traversing, text="Level-order Display", font=("Times New Roman", 13, "bold"), bg="salmon", fg="black", relief=RAISED, bd=8, command=lambda: self.traversing_decision_making(4), state=NORMAL)
        self.level_order_traversal.pack(padx=3, pady=5)

        self.level_order_display = Button(make_label_frame_for_traversing, text="Level-order Concept",font=("Times New Roman", 13, "bold"), bg="salmon", fg="black", relief=RAISED, bd=8,command=self.level_order_concept_by_queue, state=NORMAL)
        self.level_order_display.pack(padx=3, pady=5)
        # Delete Button
        self.make_delete = Button(self.window, text="Delete", font=("Times New Roman", 20, "bold"), bg="salmon", fg="black", relief=RAISED, bd=8, command=lambda: self.take_input_set_up(2), state=NORMAL)
        self.make_delete.place(x=1200,y=50)

    def take_input_set_up(self,indicator):# Set up for take input value for node insert and delete
        self.input_btn_deactivation()
        self.take_input = Label(self.window, text="Enter the number to input", fg="#060412", bg="#FFEBCD", font=("Times New Roman", 10, "bold"))
        self.take_input.place(x=10, y=120)
        self.take_entry = Entry(self.window, fg="blue", bg="white", relief=SUNKEN, bd=6, width=5, font=("Times New Roman", 10, "bold"))
        self.take_entry.place(x=20, y=150)
        self.take_entry.focus()
        self.add_btn = Button(self.window, text="Add", fg="white", bg="black", relief=RAISED, bd=6, width=5, font=("Times New Roman", 8, "bold"), state=NORMAL)
        self.add_btn.place(x=60, y=150)
    ...
    def make_default_node_with_set_position(self):# For node make and set at right position in canvas
        try:
            ...
            self.make_node = self.make_canvas.create_oval(505, 85+50, 555, 135+50, width=2, fill="#21C4A6", outline="black")
            self.make_label= Label(self.window, text=self.take_entry.get(), fg="black", bg="#21C4A6", font=("Times New Roman", 12, "bold"))
    ...
    def decision_making_about_left_side_direction(self):
        try:
            ...
            make_greater_than_label = Label(self.window, text=">", fg="black", bg="white", font=("Times New Roman", 20, "bold"))
    ...
    def decision_making_about_right_side_direction(self):
        try:
            self.right_activate = 1
            self.left_activate = 0
            make_less_than_label = Label(self.window, text="<", fg="black", bg="white", font=("Times New Roman", 20, "bold"))
    ...
    def make_container_in_result_canvas(self):# In down canvas
        self.traversing_label = Label(self.result_canvas,bg="sky blue",fg="red",text="Traversing Result",font=("Times New Roman",15,"bold","italic"))
        self.traversing_label.place(x=550,y=10)
        distance_maintainer = 8
        for i in range(15):
            self.value_show.append(i)
            self.value_show[i] = Label(self.result_canvas,bg="#FFEBCD",fg="black",text=" ",width="4",height="1",font=("Times New Roman",15,"bold","italic"),relief=SUNKEN,bd=5)
            self.value_show[i].place(x= distance_maintainer,y=50)
            distance_maintainer+=91

        make_status = Label(self.result_canvas,bg="sky blue",fg="brown",text="Status",font=("Times New Roman",17,"bold","italic"))
        make_status.place(x=580,y=100)

        self.status_note = Label(self.result_canvas,bg="sky blue",fg="green",text="Play With BST",font=("Times New Roman",20,"bold","italic"))
    ...
    def left_none_right_exist(self,node):
        ...
        if left_side_existence_checking_of_one_side_down_label_node == 1:
            ...
            if temp1:
                ...
                label_make = Label(self.window, text=take_temp1_val, fg="black", bg="green",
    ...
    def right_none_left_exist(self, node):
        ...
        if  right_side_existence_checking_of_one_side_down_label_node == 1:
            ...
            if temp1:
                ...
                label_make = Label(self.window, text=take_temp1_val, fg="black", bg="green", font=("Times New Roman", 12, "bold"))
                label_make.place(x=node_coord[0] + 14, y=node_coord[1] + 14)
