...
class CircularQueue:
    def __init__(self,root):
        self.window = root
        self.canvas_make = Canvas(self.window,width=1150,height=580,bg="sky blue",relief=RAISED,bd=8)
    ...
    def set_up(self):#For by default set up in canvas
        #Heading
        make_heading = Label(self.canvas_make,text="Circular Queue Visualizer",bg="sky blue",fg="indigo",font=("Times New Roman",25,"bold","italic"))
        make_heading.place(x=350,y=20)
        #Make Queue Container
        self.canvas_make.create_line(229, 250, 611+254+3, 250,width=3,fill="sky blue")
        self.canvas_make.create_line(229, 300, 611+254+3, 300, width=3,fill="sky blue")

        take_x = 190
        for i in range(11):
            make_index = Label(self.canvas_make,text=i-1,bg="sky blue",fg="blue",font=("Times New Roman",15,"bold"))
        ...
        self.rear_label = Label(self.canvas_make,text="rear",fg="black",bg="sky blue",font=("Times New Roman",20,"bold"))
        ...
        self.front_label = Label(self.canvas_make, text="front", fg="black", bg="sky blue", font=("Times New Roman", 20, "bold"))
        ...
        self.canvas_make.create_polygon(points_index,width=3,fill="#0FFF0F",outline="black")
        index_no = Label(self.canvas_make,text="Index no.",fg="black",bg="sky blue",font=("Times New Roman",15,"bold"))
    ...
    def show_result(self):
        self.status_label = Label(self.window, text="At first, root Node value inserted to the queue", fg="black", bg="sky blue",font=("Times New Roman", 20, "bold"))
        self.status_label.place(x=10, y=530)
        distance_maintainer = 20
        for i in range(15):
            self.value_show.append(i)
            self.value_show[i] = Label(self.canvas_make,bg="salmon",fg="black",text=" ",width="4",height="2",font=("Times New Roman",15,"bold","italic"),relief=SUNKEN,bd=4)
            self.value_show[i].place(x= distance_maintainer,y=430)
            distance_maintainer+=70

    def box_insert(self,value):#Box insert in Queue
        self.make_box = self.canvas_make.create_rectangle(60, 402, 120, 444, width=3, fill="salmon", outline="black")
        self.make_label = Label(self.window, text=value, fg="black", bg="salmon",font=("Times New Roman", 12, "bold"))
